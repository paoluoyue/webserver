 ...
 * @LastEditors: paoluo
 * @LastEditTime: 2022-08-17 17:47:33
 */
...
private:
    ...
    void DealWrite_(HttpConn* client);          // 处理写入
    void DealRead_(HttpConn* client);           // 处理读取

    void SendError_(int fd, const char*info);   // 发送数据错误
    void ExtentTime_(HttpConn* client);         // 限制时间，连接时间
